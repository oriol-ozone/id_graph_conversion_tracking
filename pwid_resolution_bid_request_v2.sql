...
hhip_pwid AS (
  SELECT 
    ...
    slr.ad_id,
    slr.seat_name,
    slr.is_bid_request_sent,
    slr.is_b_id_d_ig,
    slr.is_b_id_ctr,
    slr.br_is_impression,
    slr.br_max_bid_net_usd
  FROM seat_level_raw slr
  INNER JOIN hhip_eu ON ip_address_prefix = slr.ip_address AND date_h = slr.date_ts
  LEFT JOIN map_cached_hhip_pwid mch ON slr.request_id = mch.request_id
  WHERE mch.request_id IS NULL -- Exclude request_ids that are already mapped
),

-- Use EXCEPT to filter unmapped request_ids
ozone_pwid AS (
  SELECT
    slr.date_ts,
    'no_hhip_found' AS ip_source,
    slr.ozone_id AS premium_web_id,
    slr.request_id,
    slr.ad_id,
    slr.seat_name,
    slr.is_bid_request_sent,

...
ROUND(SUM(SAFE_DIVIDE(br_max_bid_net_usd, 1000)) / NULLIF(COUNTIF(br_is_impression), 0) * 1000, 2) AS CPM,
  ...
  SUM(ARRAY_LENGTH(successful_donation_pixels)) AS total_successful_donations,
    -- Conversion source counts split into two columns
  SUM(conversion_source_ozone_id_count) AS conversion_source_ozone_id,
  SUM(conversion_source_hhip_count) AS conversion_source_hhip

FROM (
    SELECT *,
      -- Add helper counts for ozone_id and hhip
      ARRAY_LENGTH(ARRAY(SELECT x FROM UNNEST(conversion_sources) AS x WHERE x = 'ozone_id')) AS conversion_source_ozone_id_count,
      ARRAY_LENGTH(ARRAY(SELECT x FROM UNNEST(conversion_sources) AS x WHERE x = 'hhip')) AS conversion_source_hhip_count
    FROM `ozone-analytics-dev.analytics.038_gosh_conversion_mapping_ozoneid_hhip_allconversions_raw_table`
) AS fc

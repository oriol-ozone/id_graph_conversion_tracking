...
conversion_ip_address_agg AS (
    ...
    SUM(CASE WHEN pixel_id = '10115' THEN 1 ELSE 0 END) as GOSH_Successful_Donation
    FROM conversions_raw
    WHERE ozone_user_id NOT IN (SELECT DISTINCT ozone_user_id 
                                FROM ad_data_aggregated_oid_mapped
                                WHERE GOSH_All_Site_Visitors IS NOT NULL) -- exclude rows with ozone ids mapped in ad_data_aggregated_oid
    GROUP BY 1, 2, 3, 4
),
...
ad_data_aggregated_hhip AS (
    ...
    LEFT JOIN conversion_ip_address_agg c 
        ON ad.ip_address = c.ip_address
    WHERE ad.ozone_user_id NOT IN (SELECT DISTINCT ozone_user_id 
                                FROM ad_data_aggregated_oid_mapped
                                WHERE GOSH_All_Site_Visitors IS NOT NULL) -- exclude rows with ozone ids mapped in ad_data_aggregated_oid
    GROUP BY 
        date_ts,
...
ad_data_aggregated_hhip_mapped AS (
    ...
    FROM ad_data_aggregated_hhip ad

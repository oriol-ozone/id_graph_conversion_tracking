...
conversions_ozone_id_count AS (
  SELECT 
    ozone_user_id,
    COUNT(*) AS conversion_count
  FROM conversions_raw
  GROUP BY ozone_user_id
  HAVING COUNT(*) > 1
  ORDER BY conversion_count DESC
),

conversions_raw_addressable AS (
  SELECT 
  date_ts,
  ip_address,
  ozone_user_id,
  ...
  pixel_name
  FROM conversions_raw 
  WHERE ozone_user_id IN (SELECT DISTINCT ozone_user_id FROM conversions_ozone_id_count)
),

-- create separate table with conversion totals to join at the end

conversion_totals_agg AS (
    SELECT 
    DATE_SUB(CURRENT_DATE(), INTERVAL 30 DAY) as date_ts,
    BOOL(null) as is_household,
    STRING(null) as conversion_mapping_source,
    0 as total_requests,
    0 as total_impressions,
    0 as total_revenue,
    0 as GOSH_All_Site_Visitors,
    0 as GOSH_Donation_Started,
    0 as GOSH_Successful_Donation,
    SUM(CASE WHEN pixel_id = '10114' THEN 1 ELSE 0 END) as Total_GOSH_All_Site_Visitors,
    SUM(CASE WHEN pixel_id = '10116' THEN 1 ELSE 0 END) as Total_GOSH_Donation_Started,
    SUM(CASE WHEN pixel_id = '10115' THEN 1 ELSE 0 END) as Total_GOSH_Successful_Donation,
    REGEXP_EXTRACT(page_url, r'utm_medium=([^&]+)') AS utm_medium,
    REGEXP_EXTRACT(page_url, r'utm_source=([^&]+)') AS utm_source,
    FROM conversions_raw_addressable
    GROUP BY all
),
...
final_conversions AS (
    SELECT
        ...
        0 as Total_GOSH_All_Site_Visitors,
        0 as Total_GOSH_Donation_Started,
        0 as Total_GOSH_Successful_Donation,
    ...
    SELECT * FROM conversion_totals_agg
)
